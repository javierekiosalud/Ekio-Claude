#!/bin/bash
# LOOP NOCTURNO DE ANÁLISIS DE KLAVIYO — EKIO ELECTROSMOG ESPAÑA
set -e
...
claude -p "/klaviyo-analisis-nocturno" --settings loops/klaviyo-analytics/settings.json
...
if [ ! -f "$BRIEF_FILE" ]; then
  echo "No se generó $BRIEF_FILE — nada que subir esta noche."
  exit 1
fi

# SUBE EL RESULTADO A UNA RAMA DE REVISIÓN (NUNCA A main) PARA QUE ESTÉ
# DISPONIBLE EN EL MAC PRINCIPAL POR LA MAÑANA. NO SE FUSIONA A main HASTA
# QUE TÚ LO APRUEBES CON approve-klaviyo-brief.sh
